...
Materiaux::Materiaux(int id, QString nom, QString type, QString etat, QString quantite, QString date, int idTransaction)
{
    this->id=id;
    this->nom=nom;
    this->type=type;
...
bool Materiaux::ajouterMateriaux()
{
    ...
    if (c.createconnect())
    {
        return c.insertDataM(nom, type, etat, quantite, date, idTransaction);
...
void Materiaux::afficherMateriaux(QTableWidget *tableWidget)
{
    QSqlDatabase db = QSqlDatabase::addDatabase("QODBC");
        ...
        if (query.exec(str))
        {
            ...
            QStringList labelsM;
            labelsM<<"ID"<<"Nom"<<"Type"<<"Etat"<<"Quantite"<<"Date"<<"idTransaction";
            ...
            while(query.next())
            {
                ...
                QTableWidgetItem *idTransaction= new QTableWidgetItem;
                ...
                idTransaction->setText(query.value(6).toString());
                ...
                tableWidget->setItem(RowNumber,6,idTransaction);
                RowNumber++;
            }
        }
        db.close();
...
bool Materiaux::modifierMateriaux(int id, const QString& nom, const QString& type, const QString& etat, const QString& quantite, const QString& date, int idTransaction)
{
    QSqlQuery query;

        query.prepare("UPDATE MATERIAUX SET NOM = :nom, TYPE = :type, ETAT = :etat, QUANTITE = :quantite, DATE_MISE = :date, ID_TRANSACTION = :idTransaction WHERE ID_MATERIEL = :id");
        query.bindValue(":id", id);
        ...
        query.bindValue(":idTransaction", idTransaction); // Liaison de idTransaction
...
void Materiaux::set_idTransaction(int n) { idTransaction = n; } // Nouvelle fonction pour setter idTransaction
...
int Materiaux::get_idTransaction() { return idTransaction; } // Nouvelle fonction pour getter idTransaction
