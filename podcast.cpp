...
{
    QSqlQuery query;
    query.prepare("DELETE FROM PODCASTS WHERE id_podcast= ?");
...
{
    QSqlQuery query;
    query.prepare("UPDATE PODCASTS SET nom = :newNom, duree = :duree, lieu = :lieu, categorie = :categorie, date_pod = :date_pod WHERE id_podcast = :id");
